...
function buildTimeAxisProps(ticks) {
  return {
    dataKey:       "timestamp",
    ticks:         ticks,
    tickFormatter: formatHourTick,   // HH:MM only — cleaner than HH:MM:SS
    interval:      0,
    minTickGap:    48,
    tickMargin:    6,
...
export default function Dashboard() {
  ...
  /**
   * DATA: ALL_DATA filtered by the date pill and side pill.
   * This is the "full resolution" source for aggregate stats (row counts,
   * netMBO). Chart memos derive from VIEWED_DATA, not DATA directly.
   */
  const DATA = useMemo(() => {
  ...
  var timeTicks      = getHourlyTicks(activeSlice);
  ...
  /** Changing the date resets zoom — old timestamps don't map to new session */
      ...
      {activeTab === "overview" && overviewChartData && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
          ...
          <Panel
            ...
            subtitle={isZoomed ? "ES vs SPX · zoomed" : "ES vs SPX · drag to zoom"}
            span={2}
          >
            <ResponsiveContainer width="100%" height={200}>
              ...
              >
                ...
                {isDragging && dragLo && dragHi && (
                  <ReferenceArea x1={dragLo} x2={dragHi} fill={C.accent5} fillOpacity={0.12} stroke={C.accent5} strokeOpacity={0.5} strokeWidth={1} />
                )}
            ...
            {!isZoomed && (
              <div style={{ fontSize: 9, color: C.muted, letterSpacing: "0.06em" }}>
                CLICK &amp; DRAG TO ZOOM — APPLIES TO ALL CHARTS
              </div>
            )}
          </Panel>
          ...
          <Panel title={<GreekLabel name="delta" label="Delta Evolution" />} subtitle="Call & Put delta vs time">
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={overviewChartData.delta}>
                <CartesianGrid {...GRID_PROPS} />
                <XAxis {...timeAxisProps} />
          ...
          <Panel title="MBO Net Flow" subtitle="Stacking / pulling net signal per second" span={2}>
            <ResponsiveContainer width="100%" height={160}>
              <BarChart data={overviewChartData.mboBar}>
                <CartesianGrid {...GRID_PROPS} />
                <XAxis {...timeAxisProps} />
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
          ...
          <Panel title="MBO Net Flow" subtitle="Stacking / pulling net signal per second" span={3}>
            <ResponsiveContainer width="100%" height={150}>
              <BarChart data={microChartData.mboBar}>
                <CartesianGrid {...GRID_PROPS} />
                <XAxis {...timeAxisProps} />
                <YAxis {...buildYAxisProps(55)} />
                <ReferenceLine y={0} stroke={C.muted} strokeWidth={1} />
            ...
            return (
              <Panel key={idx} title={"Book Levels " + labelA + " & " + labelB} subtitle="Net order flow over time">
                <ResponsiveContainer width="100%" height={170}>
                  <LineChart data={orderBookChartData}>
                    <CartesianGrid {...GRID_PROPS} />
                    <XAxis {...timeAxisProps} />
