// Version 4 — pure-JS parquet via hyparquet (no WASM, no workers)
...
// ── Parquet Data URL ──────────────────────────────────────────────────────────
const PARQUET_URL =
  "https://pub-73edacec404b41a29ac6cf15672e387f.r2.dev/fixed_output_agg.parquet";
...
const GREEK_DEFINITIONS = {
  delta:
    "Change in option premium per $1 move in the underlying. Calls range 0 to 1, Puts -1 to 0. Approaches +/-0.5 at-the-money.",
  gamma:
    "Rate of change of delta per $1 move in the underlying. Highest at-the-money; falls as the option moves further in or out of the money.",
  theta:
    "Daily option value decay as expiration approaches. Almost always negative for long options; accelerates near expiry.",
  ...
  vanna:
    "Rate of change of delta w.r.t. IV — equivalently, rate of change of vega w.r.t. underlying price.",
  charm:
    "Rate of change of delta over time (delta decay). Measures how delta shifts as expiration approaches.",
  vomma:
    "Rate of change of vega w.r.t. IV. High vomma means vega accelerates as volatility rises (vega convexity).",
};
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    async function loadParquet() {
      ...
      try {
        // ── 1. Fetch the whole file (avoids range-request CORS issues) ────────
        ...
        // ── 2. Wrap as hyparquet AsyncBuffer, then decode ─────────────────────
        const file = {
          byteLength: arrayBuffer.byteLength,
          slice: (start, end) => Promise.resolve(arrayBuffer.slice(start, end)),
        };
        const rawRows = await parquetReadObjects({ file });
        if (cancelled) return;

        console.log("hyparquet returned rows:", rawRows.length);

        // ── 3. Map to dashboard field names ──────────────────────────────────
    ...
    >
      ...
      >
        ...
        >
          <StatBadge
            label="ES Price"
            value={latest.esPrice?.toFixed(1)}
            color={C.accent1}
          />
          <StatBadge
            label="SPX Price"
            value={latest.spxPrice?.toFixed(1)}
            color={C.accent4}
          />
          <StatBadge
            label="Call Delta"
            value={latest.callDelta?.toFixed(3)}
            color={C.accent3}
            greek="delta"
          />
          <StatBadge
            label="Gamma"
            value={latest.gamma?.toFixed(5)}
            color={C.accent5}
            greek="gamma"
          />
        </div>
      ...
      {activeTab === "overview" && (
        ...
        >
            ...
            <ResponsiveContainer width="100%" height={200}>
              <ComposedChart data={DATA}>
                ...
                <ReferenceLine
                  ...
                  stroke={C.muted}
                  strokeDasharray="6 3"
                  label={{ value: "Strike", fill: C.muted, fontSize: 9 }}
                />
              </ComposedChart>
            </ResponsiveContainer>
          </Panel>
          ...
          <Panel
            title={<GreekLabel name="delta" label="Delta Evolution" />}
            subtitle="Call & Put delta vs time"
            span={2}
          >
            ...
            >
              ...
              >
                <div
                  style={{
                    height: "100%",
                    width: (stackCount / DATA.length) * 100 + "%",
                    background:
      ...
      {activeTab === "greeks" && (
        ...
        >
          ...
          <Panel
            title={<GreekLabel name="vanna" label="Vanna" />}
            subtitle="Rate of change of delta w.r.t. IV"
          >
            <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={DATA}>
                <CartesianGrid strokeDasharray="3 3" stroke={C.border} />
          ...
          <Panel
            title={<GreekLabel name="charm" label="Charm" />}
            subtitle="Delta decay over time"
          >
            <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={DATA}>
                <CartesianGrid strokeDasharray="3 3" stroke={C.border} />
          ...
          <Panel
            title={<GreekLabel name="theta" label="Theta" />}
            subtitle="Time decay — calls vs puts"
          >
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={DATA}>
                <CartesianGrid strokeDasharray="3 3" stroke={C.border} />
          ...
          <Panel
            title={<GreekLabel name="vomma" label="Vomma" />}
            subtitle="Vega convexity (d2P/dσ2)"
          >
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={DATA}>
                <CartesianGrid strokeDasharray="3 3" stroke={C.border} />
                ...
                <Bar dataKey="vomma" name="Vomma">
                  {DATA.map((d, i) => (
                    <Cell key={i} fill={d.vomma >= 0 ? C.accent4 : C.sell} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          <Panel
            title={<GreekLabel name="rho" label="Rho" />}
            subtitle="Interest rate sensitivity — calls vs puts"
          >
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={DATA}>
                <CartesianGrid strokeDasharray="3 3" stroke={C.border} />
      ...
      {activeTab === "microstructure" && (
        ...
        >
          <Panel
            title="Stacking / Pulling Signal"
            subtitle="MBO order flow over time (+1 stack, -1 pull)"
            span={3}
          >
            <ResponsiveContainer width="100%" height={160}>
              <BarChart data={DATA}>
                ...
                <Bar dataKey="mbo_ps" name="Stack / Pull">
                  {DATA.map((d, i) => (
                    <Cell key={i} fill={d.mbo_ps > 0 ? C.buy : C.sell} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          <Panel
            title={<GreekLabel name="delta" label="Delta vs MBO Signal" />}
            subtitle="Call delta colored by stack/pull"
            span={2}
          >
            <ResponsiveContainer width="100%" height={200}>
              <ScatterChart>
          ...
          <Panel
            ...
            subtitle="Ask vs Bid observations"
          >
            <div
              style={{
                display: "flex",
            ...
            >
              {["Ask", "Bid"].map((side) => {
                ...
                return (
                  <div key={side}>
                    ...
                    >
                      <span style={{ color: side === "Ask" ? C.sell : C.buy }}>
                        {side}
                      </span>
                      <span style={{ color: C.text }}>
                        {count} ({pct.toFixed(0)}%)
                    ...
                    >
                      <div
                        style={{
                          height: "100%",
                          width: pct + "%",
                          background: side === "Ask" ? C.sell : C.buy,
                          borderRadius: 4,
                        }}
                      />
                    </div>
              ...
              <div style={{ marginTop: 8, fontSize: 10, color: C.textDim }}>
                <GreekLabel name="gamma" label="Gamma" /> at latest:{" "}
                <span style={{ color: C.accent4 }}>
                  {latest.gamma?.toFixed(5)}
                </span>
              </div>
              <div style={{ fontSize: 10, color: C.textDim }}>
                <GreekLabel name="vega" label="Vega" /> at latest:{" "}
                <span style={{ color: C.accent3 }}>
                  {latest.vega?.toFixed(3)}
                </span>
              </div>
            </div>
          </Panel>
      ...
      {activeTab === "order book" && (
        ...
        >
          <Panel
            title="MBO Level Snapshot"
            subtitle="Signed order quantity by book level (latest)"
            span={2}
          >
                ...
                <Bar dataKey="value" name="Order Size">
                  {mboHeatData.map((d, i) => (
                    <Cell
                      ...
                      opacity={0.75 + (Math.abs(d.value) / 600) * 0.25}
                    />
                  ))}
                </Bar>
              </BarChart>
          ...
          {[
            ...
            <Panel
              key={idx}
              title={"Book Levels " + a.toUpperCase() + " & " + b.toUpperCase()}
              subtitle="Signed order flow over time"
            >
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={DATA}>
                  <CartesianGrid strokeDasharray="3 3" stroke={C.border} />
